...
import { MemorySnapshotStore } from '../../src/store/memory/MemorySnapshotStore.js';
import { makeObservedNode, makeRoot } from '../store/memory/helpers.js';
...
describe('DefaultAligner', () => {
  it('builds alignment rows by vpath', () => {
    ...
    p1.recordCoverage({ runId: run1.runId, scopes: [{ baseVPath: '/', mode: ScopeMode.FULL_SUBTREE }] });
    p1.commit();
    ...
    p2.recordCoverage({ runId: run2.runId, scopes: [{ baseVPath: '/', mode: ScopeMode.FULL_SUBTREE }] });
    p2.commit();
