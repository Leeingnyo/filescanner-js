...
3. Snapshots with incremental patching (partial subtree).
...
Windows:
- `WindowsFileId.volumeId` MUST be a volume GUID formatted as lowercase hex with hyphens and no braces, e.g. `2f1c0a1b-4c2d-4b3a-9e2a-0123456789ab`.
...
struct Coverage { runId: RunId, scopes: ScanScope[] }
...
### 9.4 Archive emission rule (normative)
...
interface SnapshotStore {
  ...
  // Purges tombstones (isDeleted=true) older than deletedBefore, optionally limited to a vpath prefix.
  // Returns number of nodes purged.
...
interface PatchSession {
  ...
  // commit MUST finalize deletion reconciliation for recorded coverage (see 10.4).
...
2. The store MUST reconcile deletions **within each covered scope** as part of `commit()`:
...
5. Deleted nodes MUST remain in the store as tombstones unless an implementation provides a purge mechanism (Appendix C).
6. Default query behavior MUST exclude deleted nodes unless `includeDeleted=true`.

This ensures snapshots do not retain ghost nodes after rescans.
...
struct MatchResult { verdict: Verdict, confidence: Confidence, evidence: Evidence[] }
```
...
interface Comparer {
  // Compares two snapshots in the same VPath namespace (root‑aligned).
  compare(leftSnapshotId: SnapshotId, rightSnapshotId: SnapshotId, opts: CompareOptions): DiffResult

  // Compares two subtrees whose bases may be in different namespaces (e.g., OS dir vs archive root).
  // The comparison namespace is relative to each base ref (base acts as "/").
  // This method MAY be used with the same snapshot on both sides.
  compareSubtree(
...
A snapshot covers a requested scope `S` iff `snapshot.lastCoverage.scopes` contains a scope that fully covers it.
...
- `COMPOSITE(parts)`: compute each part above, then join with the ASCII unit separator ``.
...
- However, when a store derives these columns, it MUST follow the definitions in this spec:
  ...
  - `parentKey` and `nameKey` MUST be derived deterministically as follows:
    - `parentKey = "pk:" + sha256(utf8(toCanonicalString(parentRef)))` where `parentRef` is the parent NodeRef of the node.
...
1. Keep tombstones for at least **N days** or **M runs** (implementation‑defined) so apps can display recent deletions.
